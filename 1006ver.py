...
def main():
    ...
    # expo=10
    mtxs = [
        np.array([[1.96407582660e+03,0.00000000e+00,1.30734941827e+03],
        [0.00000000e+00,1.96109473138e+03,7.1258603774e+02],
        [0.00000000e+00,0.00000000e+00,1.00000000e+00]]),
        np.array([[2.04123055947e+03,0.00000000e+00,1.32829370538e+03],
        [0.00000000e+00,2.04679026768e+03,5.4668366265e+02],
        [0.00000000e+00,0.00000000e+00,1.00000000e+00]]),
    ]

    dists = [
        np.array([0.08100940,-0.04600870,-0.00947654,-0.00119316,-0.12417034]),
        np.array([0.,-0.04600870,-0.00947654,-0.00119316,-0.12417034]),
    ]
    mtx=mtxs[camera]
    dist=dists[camera]
...
def auto_rotate(img,rank,rotate_num,number,path,circle_number):
    ...
    up_90=sortedUp[int(sortedUp.size*0.90)]
    down_90=sortedDown[int(sortedDown.size*0.90)]
    up_05=sortedUp[int(sortedUp.size*0.05)]
    down_05=sortedDown[int(sortedDown.size*0.05)]
    up_10=sortedUp[int(sortedUp.size*0.10)]
    down_10=sortedDown[int(sortedDown.size*0.10)]
    # varup=np.var(ROIup)
    # vardown=np.var(ROIdown)
    scoreup=float(up_90)-float(up_05)+float(up_90)-float(up_10)
    scoredown=float(down_90)-float(down_05)+float(down_90)-float(down_10)
    # if abs(scoredown-scoreup)>=20:
